...
export const CartDeleteItem = ({ item, fields }: CartDeleteItemProps) => {
  ...
  return (
    <Button
      ...
      className="text-cream-600 hover:text-terracotta-600 transition-colors ml-2"
      variant="transparent"
      size="fit"
    >
      <Trash />
    ...
    <Drawer open={isOpen} onOpenChange={(open) => (open ? openCart() : closeCart())}>
      <DrawerTrigger asChild>
        <button className="relative flex items-center justify-center text-neutral-700 hover:text-neutral-900 transition-colors">
          ...
          {itemCount > 0 && (
            <span className="absolute -top-1 -right-1 bg-terracotta-600 text-white text-xs rounded-full w-4 h-4 flex items-center justify-center">
              {itemCount}
            </span>
          )}
        </button>
