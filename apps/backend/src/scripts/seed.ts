...
export default async function seedDemoData({ container }: ExecArgs) {
  ...
  // Get categories for product linking
  const { data: categories } = await query.graph({
    entity: "product_category",
    fields: ["id", "handle"],
    filters: { handle: ["moisturizers", "serums"] },
  });
  const moisturizersCategory = categories.find((c: any) => c.handle === "moisturizers");
  const serumsCategory = categories.find((c: any) => c.handle === "serums");
  ...
  const productsToCreate = [
    {
      ...
      is_giftcard: false,
      discountable: true,
      category_ids: moisturizersCategory ? [moisturizersCategory.id] : [],
    ...
    },
    {
      ...
      is_giftcard: false,
      discountable: true,
      category_ids: serumsCategory ? [serumsCategory.id] : [],
