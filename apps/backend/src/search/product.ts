/**
 * TEMPORARY — remove once Medusa ships a built-in product search index.
 *
 ...
 * currency. `usd` is this project's default store currency. Prices are resolved
 * through the Pricing Module for this currency, which is what makes
 * `original_price` differ from `min_price` when a price list applies. A
 * storefront selling in another currency can still filter and sort on these,
 * but must render amounts from the product API, not the index.
 */
const PRICE_CURRENCY_CODE = "usd"
 ...
 * `["Size:S", "Color:Red"]`. One field keeps the index simple; a consumer
...
function toDocument(product: ProductRow): ProductDocument {
  ...
  return {
    id: product.id,
    // Nulls are fine on these: they're read, not faceted.
...
export default defineSearchIndex({
  ...
  async consume(event, { container }) {
    ...
    return [{ action: "upsert", documents: products.map(toDocument) }]
  },
